...
def PlacingSecondTile(AssemblyHistory, BaseStates):
        ...
        for rule in tileTransitions:
            ...
            if(tileLeftNeighbor == None and tileRightNeighbor == None):#If the tile has no neighbors, then there's no way to make a transition, so skip the entire transition check
                ...
                if(tileLabel == ruleOriginalLeft and tileRightNeighborLabel == ruleOriginalRight):
                    tempMove = AvailableTransition(ruleOriginalLeft, ruleOriginalRight, ruleFinalLeft, ruleFinalRight, tileID, tileRightNeighborID)
                    AvailableMoves.append(tempMove)
                ...
                if(tileLabel == ruleOriginalRight and tileLeftNeighborLabel == ruleOriginalLeft):
                    tempMove = AvailableTransition(ruleOriginalLeft, ruleOriginalRight, ruleFinalLeft, ruleFinalRight, tileID, tileLeftNeighborID)
                    AvailableMoves.append(tempMove)
    
    #Main: Execute a Random Available Move
    elementNum = randrange(0, len(AvailableMoves)) #Pick a number
    activeMove = AvailableMoves[elementNum] #Pick the corresponding available move from that number
    ...
    if(moveType == "Affinity"):
        ...
        destinationTile = ActiveTile(BaseStates[0], 0, 0) #Dummy Data to Prevent Future Errors
    ...
    else: #If it's a transition rule:
        print("Executing Transition Rule!")
    print("New Assembly:")
